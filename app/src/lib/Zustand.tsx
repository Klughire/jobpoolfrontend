...
const useStore = create<StoreState>((set) => ({
  ...
  login: (token: string, user: UserData) => {
    ...
    try {
      ...
      if (decoded && decoded.userId !== undefined) {
        ...
        console.log("Login successful:", { userId: decoded.userId, user });
      } else {
        console.error("Invalid token payload", decoded);
      }
    } catch (error) {
  ...
  logout: () => {
    ...
    if (typeof window !== "undefined") {
      localStorage.removeItem("token");
      localStorage.removeItem("user");
      localStorage.removeItem("bids"); // Clear bids as in TaskDetailPage
    }
    ...
    if (token && user) {
      try {
        ...
        if (decoded && decoded.userId !== undefined && parsedUser) {
          ...
          console.log("Auth restored:", { userId: decoded.userId, user: parsedUser });
        } else {
